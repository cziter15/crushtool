...
function Resolve-CrushConfigPath {
    $cwd = Get-Location
    ...
    if ($scriptDir) {
        $searchPaths.Add((Join-Path $scriptDir ".crush.json"))
        $searchPaths.Add((Join-Path $scriptDir "crush.json"))
        $searchPaths.Add((Join-Path (Split-Path -Parent $scriptDir) ".crush.json"))
        $searchPaths.Add((Join-Path (Split-Path -Parent $scriptDir) "crush.json"))
    }
    ...
    # Fallback: Default to global path if no existing file is found anywhere
    if ($globalPath) {
        return $globalPath
    }

    return Join-Path $cwd "crush.json"
}

 $configPath = Resolve-CrushConfigPath
...
function Save-Config {
    ...
    # Ensure target directory path exists before saving
    ...
    if ($null -ne $Config -and $null -ne $Config.providers) {
        foreach ($p in $Config.providers.PSObject.Properties) {
            if ($null -ne $p.Value -and $p.Value -is [PSCustomObject]) {
                $modelsVal = if ($null -ne $p.Value.models) {
                    [object[]]@($p.Value.models)
                } else {
                    [object[]]@()
                }
                $p.Value | Add-Member `
                    -NotePropertyName "models" `
                    -NotePropertyValue $modelsVal `
                    -Force
            }
        }
    }
...
function Refresh-Overview {
    ...
    $script:configPath = Resolve-CrushConfigPath
...
function Sync-Models {
    ...
    # Spawning runspaces avoids PowerShell.exe startup overhead.
    # The resolved config path is passed explicitly down to crushtool.ps1.
    $rs = [runspacefactory]::CreateRunspace()
    $rs.Open()
    $ps = [powershell]::Create()
    $ps.Runspace = $rs
    [void]$ps.AddScript({
        param($toolPath, $prov, $u, $cfgPath)
        & $toolPath -add_update -provider $prov -url $u -config $cfgPath 2>&1 | Out-String
    }).AddArgument($crushtoolPath).AddArgument($pName).AddArgument($pUrl).AddArgument($configPath)
    ...
    $script:syncTimer.Add_Tick({
        ...
        $hadErrors = $script:syncPs.HadErrors
        $script:syncPs.Dispose()
        $script:syncRs.Dispose()

        $outputTextBox.Text = $output

        if ($hadErrors -or $output -match "^Error") {
...
function Open-ConfigFile {
    if (Test-Path $configPath) {
        ...
        $confirm = [System.Windows.Forms.MessageBox]::Show(
            "Config file not found at:`n$configPath`n`nWould you like to initialize a new configuration file at this path?",
            "Initialize Config", "YesNo", "Question"
        )
        if ($confirm -eq "Yes") {
            try {
                ...
                Save-Config -Config $emptyConfig -Path $configPath
                Refresh-Overview
                Load-Models
                Start-Process $configPath
            } catch {
                [void][System.Windows.Forms.MessageBox]::Show(
                    "Failed to create file:`n$_", "Error", "OK", "Error"
                )
            }
        }
    }
